...
# 2. Localizar Hosted Zone (Produção)
provision_zone() {
    log "Finding Hosted Zone for $DOMAIN"
    local zone_id
    
    # Tenta encontrar a zona exata ou a zona pai (ex: appsrom.click)
    ...
    if [[ -z "$zone_id" || "$zone_id" == "None" ]]; then
        ...
        log "Exact zone not found, trying parent domain: $parent_domain"
        zone_id=$(aws route53 list-hosted-zones-by-name --dns-name "$parent_domain" --query "HostedZones[?Name=='$parent_domain.'].Id" --output text | cut -d'/' -f3)
    fi

    if [[ -z "$zone_id" || "$zone_id" == "None" ]]; then
        error "Hosted Zone not found for $DOMAIN or $parent_domain in this account."
    fi

    log "Hosted Zone found: $zone_id"
